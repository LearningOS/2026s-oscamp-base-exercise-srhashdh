...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            if (pte & PTE_V) == 0 {
                ...
                self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn] = (new_ppn << PPN_SHIFT) | PTE_V;
                curr_ppn = new_ppn;
            }else {
        ...
        let last_node = self.nodes.get_mut(&curr_ppn).unwrap();
        last_node.entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let mut curr_ppn = self.root_ppn;
        let level = 2;
        let vpn2 = Self::extract_vpn(va, level);
        let pte = self.nodes.get(&curr_ppn).unwrap().entries[vpn2];
        if pte & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
            curr_ppn = new_ppn;
        }
        else {
            curr_ppn = pte >> PPN_SHIFT;
        ...
        let last_node = self.nodes.get_mut(&curr_ppn).unwrap();
        last_node.entries[vpn1] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    }
}
