...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        for ety in &self.entries {
            if ety.valid && ety.vpn == vpn && ety.asid == asid {
                self.stats.hits += 1;
                return Some(ety.ppn);
            }
        }
    ...
    pub fn flush_all(&mut self) {
        ...
        self.entries.iter_mut().for_each(|ety| ety.valid = false);
    }
    ...
    pub fn flush_by_vpn(&mut self, vpn: u64) {
        ...
        self.entries
            .iter_mut()
            .filter(|ety| ety.valid && ety.vpn == vpn)
            .for_each(|ety| ety.valid = false);
    }
    ...
    pub fn flush_by_asid(&mut self, asid: u16) {
        ...
        self.entries
            .iter_mut()
            .filter(|ety| ety.valid && ety.asid == asid)
            .for_each(|ety| ety.valid = false);
    }
    ...
    pub fn valid_count(&self) -> usize {
        ...
        self.entries.iter().filter(|ety| ety.valid).count()
    }
}
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        self.tlb.lookup(vpn, self.current_asid)
                .or_else(||{
                    self.page_table
                        .iter()
